...
- **Port already in use, or Docker reports `Bind for 0.0.0.0:80 failed: port is already allocated` even though nothing is listening on it (`ss`/`docker ps` both clean)** — either something else is genuinely using that port, or Docker's own internal port allocator has gotten out of sync (confirmed via `journalctl -u docker`: it fails inside dockerd itself, before any real iptables rule is ever created). Set `FRONTEND_PORT=<some other port>` (e.g. `8080`) in `.env` and re-run `docker compose up -d`, or as a first try just re-run `docker compose up -d` again unchanged. `install.sh` handles this automatically: it retries with a short settle delay, falls back through `8080`/`8888`/`8081`, and if all of those still fail it restarts the Docker daemon (which clears the allocator's in-memory state) and tries once more.
