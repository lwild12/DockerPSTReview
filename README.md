...
This installs Docker if it isn't already present, clones the repo to `/opt/DockerPSTReview`, generates a `.env` with real random secrets (never overwriting one that already exists, so it's safe to re-run), and brings the whole stack up. Along the way it asks two quick sizing questions — how many documents to render/parse in parallel per import — suggesting defaults based on this machine's CPU/RAM; just press Enter to accept them. It prints the URLs to use when it's done. Skip straight to [step 6](#6-your-first-case-start-to-finish) below to create your first account and case — everything before that is what the script just did for you.
...
The very first account ever registered on the instance automatically becomes a system **superuser** (full `/admin` access — user management and system settings). Register before exposing the instance to anyone else. A superuser can later disable further self-registration from `/admin` (see [Admin UI and OIDC login](#admin-ui-and-oidc-login)) once your team is set up.
...
4. **Review analytics** (optional, between steps 2 and 3) — click **"Compute"**/**"Recompute"** on the case page to find near-duplicate documents and flag redundant thread messages (ones whose content is already fully quoted in a later message). This doesn't run automatically — re-run it after importing more PSTs into the case.
5. **Step 3 — Add documents to review** — click **"Add all documents to a review set"** to put every unique document into a new or existing review set in one action (with checkboxes to exclude redundant thread messages and near-duplicates, using the analytics above), or **"Hand-pick documents instead"** to filter/search the full document list (by type, keyword — including OCR'd text from scanned attachments — custodian, thread) and add a specific subset.
6. **Step 4 — Review** — open a review set to tag documents, fill in any custom **Coding Fields** your case defines (per-case review metadata, managed from the case's "Coding Fields" page), redact anything sensitive (drag rectangles on the rendered PDF — saves immediately, nothing is permanently altered until export), and set each document's review status (unreviewed / in review / reviewed / flagged).
7. **Export** — from step 4, click **"Export a review set →"**, pick the set, choose **Bates-numbered production set** (one PDF per document + a Bates log CSV, zipped) or **single combined PDF**, optionally set a Bates prefix/start number, and submit. Redactions are burned in (genuinely removed, not just covered) at this point. Download once the job shows **completed**.
8. **Audit log** — as an admin, click **"Audit log"** at the top of the case to see a record of every import, tag, redaction, review-set, and export action taken on it, with who did it.
...
**To update a self-hosted install** (one you set up with `install.sh`, or any production-style deployment using `docker-compose.yml` on its own): run `update.sh` from the repo directory (`sudo ./update.sh`, or `make update`) — it pulls the latest code, rebuilds only what changed, and restarts, all without touching your `.env`/secrets or your existing data volumes. It also warns you if a new setting has shown up in `.env.example` that isn't in your `.env` yet, and will ask the same `RENDER_CONCURRENCY`/`PARSE_CONCURRENCY` sizing questions `install.sh` does if your `.env` predates them — only once, never again after they're set. There's no separate migrate command to remember — new database migrations run automatically as part of the restart.
...
- **Port already in use, or Docker reports `Bind for 0.0.0.0:80 failed: port is already allocated`** — most likely something else on your machine is genuinely using that port. Set `FRONTEND_PORT=<some other port>` (e.g. `8080`) in `.env` and re-run `docker compose up -d`. `install.sh` also retries automatically through `8080`/`8888`/`8081` if this happens. If you're running `docker compose up` directly rather than through `install.sh`, make sure you're not unintentionally loading `docker-compose.override.yml` in a production deployment (it's dev-only — it builds the Vite dev server instead of the production nginx image); run `docker compose -f docker-compose.yml up -d --build` to be explicit about excluding it.
- **Login says "Invalid email or password" right after registering** — double check the email/password match exactly (passwords are case-sensitive), and try a private/incognito window in case a stale cookie from a previous install is interfering.
...
- **System settings** — the `ENABLE_API_DOCS`/`COOKIE_SECURE` env vars' live values (see above), an **"Allow new user registration"** toggle (turn off once your team is onboarded to hide the Register link and block new sign-ups — existing users are unaffected), plus OIDC login configuration: issuer URL, client ID, client secret, and the button label shown on the login page. None of this requires editing `.env` or restarting the stack — the client secret is encrypted at rest with `SECRET_ENCRYPTION_KEY` and never sent back to the browser after saving.
...
- `RENDER_CONCURRENCY` (default `4`) — how many documents a single import job renders (PDF/OCR) in parallel. `install.sh`/`update.sh` prompt for this, suggesting a value scaled to the machine's CPU cores and RAM.
- `PARSE_CONCURRENCY` (default `8`) — how many manifest entries (emails, contacts, calendar items) a single import job parses in parallel. Lighter per-item work than rendering, so this can usually run higher; also prompted for by `install.sh`/`update.sh`.
- `CELERY_WORKER_CONCURRENCY` (default `2`) — a separate axis from the two above: how many import/export *jobs* the `worker` container runs at once, each a full OS process. Not prompted for interactively; set it directly in `.env` if you need more than one PST importing at a time.
